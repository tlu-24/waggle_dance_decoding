...
# Returns all rows that match on frame and cluster
dup = waggle_df[waggle_df.duplicated(subset=['frame', 'Cluster'], keep=False)]
# All non duplicated rows
# Returns all rows that match on frame and cluster
non_dup = waggle_df[~waggle_df.duplicated(
...
same_pts = [np.argwhere(i[0] == pts[:, -1])
            for i in np.array(np.unique(pts[:, -1], return_counts=True)).T if i[1] >= 2]
save_row = []
for i in same_pts:
    dist = []
    pre = min(i)-1
    for j in i:
        ...
        dist_pre = np.sqrt((pts[pre, 1]-pts[j, 1]) **
                           2 + (pts[pre, 2]-pts[j, 2])**2)
...
for i in list(df['Cluster'].unique()):
    ...
    clust.loc[:, 'euclid'] = np.sqrt(
        np.square(df['x'] - df['x'].shift(1)) + np.square(df['y'] - df['y'].shift(1)))
