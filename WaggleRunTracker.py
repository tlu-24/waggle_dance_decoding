...
# Load dataset
path = 'WaggleDetectionsCorrect154.pkl'#'WaggleDetections154-test-Cleaned.pkl'
...
for i in range(len(df['Cluster'].unique())):
    ...
    # Setup video
    counter = start
    cap = cv2.VideoCapture('framediff.mp4')#'test.mp4')
    cap.set(1, start)
    ret, frame = cap.read()
    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    if True:
        out = cv2.VideoWriter("test"+'_tracked'+str(i)+'.mp4', cv2.VideoWriter_fourcc(
    ...
    # print(clust, cv2.contourArea(contour))
    if cv2.contourArea(contour) > 1590:
        contour = findROIContour(opening, bbox)
        # print(contour)
    ...
    while counter < end:
        ...
        while contour is None or cv2.contourArea(contour) <= 80: # or too small
            ...
            opening[int(bbox[1]):int(bbox[1]+bbox[3]), int(bbox[0]):int(bbox[0]+bbox[2])] = thresh[int(bbox[1]):int(bbox[1]+bbox[3]), int(bbox[0]):int(bbox[0]+bbox[2])]
            contour = findROIContour(opening, bbox)
            if low < 60:
                break
...
final_df.to_csv('WaggleRuns.csv')